  ...
  <div>
    ...
    <el-table :data="tableData" border stripe>
      ...
      <el-table-column label="操作" width="280">
        <template slot-scope="scope" >
          <el-button type="warning" icon="el-icon-edit" @click="handleAccountAdd(scope.row)">充值</el-button>
          <el-button type="primary" icon="el-icon-edit" @click="$router.push('/editUser?id=' + scope.row.id)">编辑</el-button>
          <el-popconfirm
  ...
  data() {
    const checkNums =  (rule, value, callback) => {
      ...
      if (value === null || value === undefined) {
        return callback(new Error('请输入充值积分'));
      }
      if (!Number.isInteger(value) || value < 10 || value >= 200) {
        callback(new Error('请输入大于10小于等于200的数字值'));
      }
      callback();
    };
